...
import { cn, formatAffiliationDuration, isCurrentPosition } from '@/lib/utils'
		...
		<Link
			...
			<div className='flex items-start gap-3'>
				...
				<div className='min-w-0 flex-1'>
					...
					<p className='text-cyber-cyan truncate text-sm'>{item.affiliation.name}</p>
				</div>
				{current && (
			...
			<div className='mt-auto flex items-center justify-between pt-2 font-mono text-[0.65rem] tracking-wider uppercase'>
				<span className={cn('text-muted-foreground')}>{formatAffiliationDuration(item.startDate, item.endDate)}</span>
				<Icon
